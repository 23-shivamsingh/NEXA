...
export const ConciergeDrawer: React.FC = () => {
  ...
  const [transitioningDestination, setTransitioningDestination] = useState<string | null>(null);
      ...
      recommendations: [
        {
          ...
          spaceId: spaces[0]?.id,
          subtitle: 'Live collaborative audio synthesis & shared frequency stream',
        },
        {
            ...
            recommendations: [
              {
                ...
                view: 'matches',
                subtitle: thinker.bio || 'Exploring metaphysics and deep curiosity',
              },
            ],
          },
        ]);
        ...
        {transitioningDestination && (
          <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-white/95 dark:bg-[#0b0f22]/95 backdrop-blur-md p-6 text-center animate-in fade-in duration-200">
            ...
            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400 max-w-xs">
              Entering <span className="font-semibold text-purple-600 dark:text-purple-400">{transitioningDestination}</span>
            </p>
                ...
                {log.recommendations && log.recommendations.length > 0 && (
                  <div className="space-y-2 pt-1">
                    {log.recommendations.map((rec, i) => {
                      const linkedSpace = rec.spaceId ? spaces.find((s) => s.id === rec.spaceId) : null;
                      const linkedMoment = rec.momentId ? moments.find((m) => m.id === rec.momentId) : null;
                      ...
                      return (
                        ...
                        >
                          ...
                          <p className="mt-1 text-[11px] text-slate-500 dark:text-slate-400 line-clamp-2 leading-relaxed">
                            {rec.subtitle || linkedSpace?.description || 'Active social node'}
                          </p>
                          ...
                          {linkedSpace && (
                            <div className="mt-2.5 flex flex-wrap items-center gap-3 border-t border-slate-100 dark:border-white/5 pt-2 text-[10px] text-slate-500 dark:text-slate-400">
                              <span className="flex items-center gap-1 text-cyan-600 dark:text-cyan-400 font-medium">
                                ...
                                {linkedSpace.activeParticipants.length} synchronized
                              </span>

                              {linkedSpace.collectiveGoal && (
                                <span className="flex items-center gap-1 text-purple-600 dark:text-purple-400 font-medium">
                                  <Target className="h-3 w-3" />
                                  {linkedSpace.collectiveGoal.current}/{linkedSpace.collectiveGoal.target} {linkedSpace.collectiveGoal.unit}
                                </span>
                              )}
                            </div>
                          )}
