...
function getShortTitle(m: Moment): string {
  ...
  if (tl.includes("soundscape") || tl.includes("ambient"))
...
function getNodeIcon(m: Moment) {
  ...
  if (t.includes("music") || t.includes("synth") || t.includes("audio"))
    return Music;
  if (t.includes("radio")) return Radio;
  if (t.includes("meteor") || t.includes("space") || t.includes("stargazing"))
    return Sparkles;
  if (t.includes("code") || t.includes("rust") || t.includes("hacker"))
    return Code;
  if (t.includes("coffee") || t.includes("tea") || t.includes("focus"))
    return Coffee;
...
export const OrbitNode = React.memo(
  React.forwardRef<HTMLDivElement, OrbitNodeProps>(function OrbitNode(
    { moment, x, y, isHovered, onHover, onClick, scale = 1 },
    ref,
  ) {
    const isAnchoredInStore = useNexaStore((state) =>
      state.anchoredMomentIds.includes(moment.id),
    );
    const hasResonated = useNexaStore(
      (state) => !!state.userResonances[moment.id],
    );
    const isAnchored =
      isAnchoredInStore || moment.lifecycleStatus === "Anchored";
